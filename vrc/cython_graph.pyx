...
cdef object iter_to_python(cgraph.Iter *i):
    it = Iter()
    it.i = i
    return it
...
cdef class Node:
    ...
    def name(self) -> typing.Optional[str]:
        if self._name is None:
            self._name = c_string_decode(cgraph.graph_name_by_index(NULL, self.g, self.index))
            self._username = c_string_decode(cgraph.graph_username_by_index(NULL, self.g, self.index))
    ...
    def file(self) -> typing.Optional[str]:
        ...
        if self._line is None:
            self._file = c_string_decode(cgraph.graph_location_by_index(NULL, self.g, self.index, &line))
            self._line = line
...
cdef class Graph:
    ...
    def _get_callers(self, i: int) -> typing.Iterable[int]:
        return iter_to_python(cgraph.graph_get_callers(NULL, self.g, i))

    def _get_callees(self, i: int) -> typing.Iterable[int]:
        yield from iter_to_python(cgraph.graph_get_callees(NULL, self.g, i))
        yield from iter_to_python(cgraph.graph_get_refs(NULL, self.g, i))
    ...
    def _all_nodes_for_file(self, file: str) -> typing.Iterable[int]:
        return iter_to_python(cgraph.graph_all_nodes_for_file(NULL, self.g, file.encode("utf-8")))
    ...
    def _all_nodes_for_label(self, label: str) -> typing.Iterable[int]:
        return iter_to_python(cgraph.graph_all_nodes_for_label(NULL, self.g, label.encode("utf-8")))
